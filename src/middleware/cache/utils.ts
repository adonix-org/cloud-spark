...
/**
 * Creates a new `Request` suitable for generating a Cloudflare cache key.
 *
 * Only the headers listed in `CACHE_REQUEST_KEY_HEADERS` are copied from the
 * original request. These are the headers that affect Cloudflare caching:
 ...
 *
 * All other headers are ignored to avoid unnecessary cache key variance.
 *
 * @param headers Request headers to filter for cache relevance.
